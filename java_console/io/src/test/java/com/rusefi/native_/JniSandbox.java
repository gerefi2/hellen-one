...
public class JniSandbox {
    ...
    private static final String LIBNAME = "_gerefi_test";
    ...
    public static void loadLibrary() {
        ...
        if (list.length == 0) {
            if (FileLog.isWindows()) {
                System.out.println("Have you invoked `make -j4 SANITIZE=no build/_gerefi_test'?");
            } else {
                System.out.println("Have you invoked `make -j4 SANITIZE=no build/lib_gerefi_test`?");
            }
        }
