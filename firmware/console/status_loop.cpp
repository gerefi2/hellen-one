...
static void printgerefiVersion(const char *engineTypeName, const char *firmwareBuildId) {
...
void printOverallStatus() {
	...
	if (printVersionTimer.hasElapsedSec(1)) {
		...
		printgerefiVersion(getEngine_type_e(engineConfiguration->engineType), FIRMWARE_ID);
