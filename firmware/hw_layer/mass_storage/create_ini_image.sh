...
# hw_layer/mass_storage/create_ini_image.sh tunerstudio/generated/gerefi.ini hw_layer/mass_storage/ramdisk_image.h  112 test https://gerefi.com/s/test
...
cp hw_layer/mass_storage/filesystem_contents/gerefi_Wiki_template.url hw_layer/mass_storage/wiki.temp
...
mcopy -i $IMAGE hw_layer/mass_storage/filesystem_contents/gerefi\ Forum.url ::
mcopy -i $IMAGE hw_layer/mass_storage/filesystem_contents/gerefi\ Quick\ Start.url ::
mcopy -i $IMAGE hw_layer/mass_storage/wiki.temp ::gerefi\ ${SHORT_BOARD_NAME}\ Wiki.url
